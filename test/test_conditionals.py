...
for m in tqdm(range(no_samples)):
    ...
    R_place = utils.sample_hyperplanes(X, omega, paths, depth, mu, tau, possible_paths, R_place)
    Rroot[:, :, m] = R_place[0]
    Rsecond[:, :, m] = R_place[1]
...
for m in tqdm(range(no_samples)):
    ...
    # Xtemp = samp.pg_kalman_batch(dim, 1, Xtemp, U, P, A, np.repeat(Q[:, :, na], K, axis=2),
    #                        Ct, S, Y, paths, Z, omega, alphas, Lambdas, R, depth)
    temps = Parallel(n_jobs=4)(delayed(samp.pg_kalman)(dim, 1, Xtemp[n], U[n], P, A, np.repeat(Q[:, :, na], K, axis=2),
                           Ct, S, Y[n], paths[n], Z[n], omega[n], alphas, Lambdas, R,
                                                       depth, marker=n) for n in range(len(Xtemp)))
    for n in range(len(temps)):
        Xtemp[temps[n][1]] = temps[n][0]
...
for idx in tqdm(range(no_realizations)):
    ...
    spikes.append(npr.binomial(Nmax, Prob_of_spike[idx]))

fig = plt.figure(figsize=(10, 10))
ax = fig.add_subplot(111)
ax.imshow(spikes[0], cmap='coolwarm')
ax.set_aspect('auto')
plt.tight_layout()
fig.show()
...
for m in tqdm(range(no_samples)):
    Ctemp = samp.emission_parameters_spike_train(spikes, X, omegay, mask,
                                                mu, Sigma, normalize=False, N=Nmax)
    C_samples[:, :, m] = Ctemp
    omegay = samp.pg_spike_train(X, Ctemp, omegay, neurons, N=Nmax)
...
Lambdas = np.repeat(20*np.eye(dim)[:, :, na], max_len, axis=2)
for m in tqdm(range(no_samples)):
    omegay = samp.pg_spike_train(X, Cspike, omegay, neurons, N=Nmax)
    omega = samp.pg_tree_posterior(Xtemp, omega, R, paths, depth)
    # Xtemp = samp.pg_kalman_batch(dim, 1, Xtemp, U, P, A, np.repeat(Q[:, :, na], K, axis=2),
    #                              Cspike, 0, spikes, paths, Z, omega, alphas, Lambdas, R, depth, bern=True, omegay=omegay,
    #                              N=Nmax)
    temps = Parallel(n_jobs=4)(delayed(samp.pg_kalman)(dim, 1, Xtemp[n], U[n], P, A, np.repeat(Q[:, :, na], K, axis=2),
                                                       Cspike, 0, spikes[n], paths[n], Z[n], omega[n], alphas, Lambdas, R,
                                                       depth, marker=n, bern=True, omegay=omegay[n],
                                                       N=Nmax) for n in range(len(Xtemp)))
    for n in range(len(temps)):
        Xtemp[temps[n][1]] = temps[n][0]
    X_samples.append(Xtemp)
    X_samples.append(Xtemp)
