...
#                alpha = np.matmul(Lambda, (np.linalg.solve(P[:, :, t], X[idx][:, t]) + temp_mu.flatten()))
...
#            x_prior = np.matmul(As[:, :-D_bias, int(Z[idx][t])], alpha[:, na]) + \
#                      np.matmul(As[:, -D_bias:, int(Z[idx][t])], U[idx][:, t])
        ...
        for t in range(X[idx][0, :].size - 2, -1, -1):
            ...
            # Sample
            # X[idx][:, t] = np.random.multivariate_normal(np.array(mu_n).ravel(), Pn)
            X[idx][:, t] = (mu_n + np.linalg.cholesky(Pn) @ npr.normal(size=D_in)[:, na]).ravel()

    return X

# In[10]:
def pg_kalman_spike(D_in, D_bias, X, U, P, As, Qs, C, Y, paths, Z, omega,
      omegay, alphas, Lambdas, R, depth):
    '''
    Polya-Gamma augmented kalman filter for sampling the continuous latent states
    :param D_in: dimension of latent space
    :param D_bias: dimension of input
    ...
    for idx in range(len(X)):
        for t in range(X[idx][0, :].size - 1):
            ...
            K = np.matmul(P_prior, np.linalg.solve(np.matmul(np.matmul(C[:, :-1], P_prior),
                                                             C[:, :-1].T) + S, C[:, :-1]).T)
            # Correction of estimate
#            X[idx][:, t + 1] = x_prior + np.matmul(K,  (np.linalg.solve(W, kt) - np.matmul(C[:, :-1], x_prior) - C[:, :-1]))
        ...
        """
        # X[idx][:, -1] = np.random.multivariate_normal(np.array(X[idx][:, -1]).ravel(), P[:, :, X[idx][0, :].size - 1])
        X[idx][:, -1] = X[idx][:, -1]  + (np.linalg.cholesky(P[:, :, X[idx][0, :].size - 1]) @ npr.normal(size=D_in)[:, na]).ravel()
        for t in range(X[idx][0, :].size - 2, -1, -1):
            # Load in alpha and lambda
            alpha = alphas[:, t]
            Lambda = Lambdas[:, :, t]
